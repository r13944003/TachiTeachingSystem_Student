...
namespace RuntimeHandle
{
    ...
    public class ScaleAxis : HandleBase
    {
        ...
        public ScaleAxis Initialize(RuntimeTransformHandle p_parentTransformHandle, Vector3 p_axis, Color p_color, float p_handleSize)
        {
            ...
            mf.mesh = MeshUtils.CreateCone(p_axis.magnitude * SIZE * _handleSize, .02f*_handleSize, .02f*_handleSize, 8, 1);
            MeshCollider mc = o.AddComponent<MeshCollider>();
            mc.sharedMesh = MeshUtils.CreateCone(p_axis.magnitude * SIZE * _handleSize, .1f * _handleSize, .02f * _handleSize, 8, 1);
            ...
            mf.mesh = MeshUtils.CreateBox(.25f * _handleSize, .25f * _handleSize, .25f* _handleSize);
            mc = o.AddComponent<MeshCollider>();
            o.transform.localRotation = Quaternion.FromToRotation(Vector3.up, p_axis);
            o.transform.localPosition = p_axis * SIZE * _handleSize;
        ...
        protected void Update()
        {
            ...
            transform.GetChild(1).localPosition = _axis * (SIZE * (1 + delta)) * _handleSize;
        }
