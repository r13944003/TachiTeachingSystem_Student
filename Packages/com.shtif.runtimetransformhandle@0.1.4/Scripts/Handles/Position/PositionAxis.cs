...
namespace RuntimeHandle
{
    ...
    public class PositionAxis : HandleBase
    {
        ...
        public PositionAxis Initialize(RuntimeTransformHandle p_runtimeHandle, Vector3 p_axis, Color p_color, float p_handleSize = 0.3f)
        {
            ...
            mf.mesh = MeshUtils.CreateCone(2f*p_handleSize, .02f*p_handleSize, .02f*p_handleSize, 8, 1);
            MeshCollider mc = o.AddComponent<MeshCollider>();
            mc.sharedMesh = MeshUtils.CreateCone(2f*p_handleSize, .1f*p_handleSize, .02f*p_handleSize, 8, 1);
            ...
            mf.mesh = MeshUtils.CreateCone(.4f*p_handleSize, .1f*p_handleSize, .0f, 8, 1);
            mc = o.AddComponent<MeshCollider>();
            o.transform.localRotation = Quaternion.FromToRotation(Vector3.up, _axis);
            o.transform.localPosition = p_axis * p_handleSize * 2;
